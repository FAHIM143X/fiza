...
//
// This program is distributed under the terms of the GNU Affero General Public License (AGPLv3).
// You may use, modify, and redistribute it under that license.
...
// Here the flags, video quality, etc. are chosen. See the yt-dlp documentation
const formats = {
...
const updateYtDlp = async (m, errorMsg = null) => {
  try {
    ...
    const errorOutput = error.stdout || error.stderr || `Error updating: ${error.message}`;
...
const uploadCookies = async (m, cookieText = null) => {
  try {
    ...
    if (cookieText) {
      ...
      if (!media) {
        await m.reply('Error downloading the cookies file');
        return;
      }
      cookieContent = media.toString();
    } else {
      await m.reply('Cookies must be text or a file. See the tutorial here: https://youtu.be/KUk9nEf00_U');
      return;
    }
    ...
    await m.reply(`Cookies uploaded successfully to ${config.cookiesFile}`);
  } catch (error) {
...
const processDownloadedFile = async (m, filePath, originalFileName, isVideo = false) => {
  try {
    ...
    console.error(`PLUGIN DLA Error processing file ${filePath}: ${error.message}`);
    throw error;
  }
};
...
const downloadWithYtDlp = async (m, urls, customOptions = '', enablePlaylist = false, isVideo = false) => {
  ...
  try {
      ...
      try {
        ...
        for (const file of files) {
          ...
          try {
            ...
            console.error(`PLUGIN DLA Error processing file ${file}: ${processError.message}`);
          }
        }
      } catch (error) {
    ...
    await fs.promises.rm(outputDir, { recursive: true, force: true }).catch((err) => {
      console.error(`PLUGIN DLA Error cleaning temp directory: ${err.message}`);
    });
  }
};
...
const searchAndDownload = async (m, searchQuery, isVideo = false) => {
  ...
  try {
    ...
    if (!success) {
      console.error(`PLUGIN DLA No results found for ${searchQuery}`);
    }
  } finally {
    await fs.promises.rm(outputDir, { recursive: true, force: true }).catch((err) => {
      console.error(`PLUGIN DLA Error cleaning temp directory: ${err.message}`);
...
const handleRequest = async (m) => {
  ...
  try {
    ...
    console.error(`PLUGIN DLA Error in dla command: ${error.message}`);
